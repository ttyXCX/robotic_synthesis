...
import h5py
from vizdoom_utils.vocab import VizDoomDSLVocab
...
def load_input_file(path_to_dataset):#, path_to_vocab):
    ...
    VizDoomDSLVocab: Object containing the vocabulary
    '''
    viz_vocab = VizDoomDSLVocab(perception_type="simple")
    tgt_tkn2idx = viz_vocab.token2int
    # special token
    tgt_tkn2idx["<pad>"] = len(tgt_tkn2idx)
    tgt_tkn2idx["<s>"] = len(tgt_tkn2idx)
    ...
    path_to_ds_cache = path_to_dataset.replace('.hdf5', '.cache')
    if os.path.exists(path_to_ds_cache):
        dataset = torch.load(path_to_ds_cache)
    else:
        data_file = h5py.File(path_to_dataset, "r")
        srcs = []
        tgts = []

        for data_group_key in data_file:
            if data_group_key == "data_info":
                continue
            
            current_ios = []
            data_group = data_file[data_group_key]
            # Get the target program
            tgt_program_idces = data_group["program"][:]
            # Get io tensor
            IOs = data_group["IO"][:]
            demo_num, _, layers, feature_len = IOs.shape

            for i in range(demo_num):
                IO = IOs[i]
                inp_grid = IO[0].flatten()
                inp_grid = torch.ShortTensor(inp_grid)

                out_grid = IO[1].flatten()
...
def shuffle_dataset(dataset, batch_size, randomize=True):
    ...
    grouped_pairs = [pairs[pos: pos + batch_size]
                     for pos in range(0,len(pairs), batch_size)] # for pos in xrange(0,len(pairs), batch_size)]
    if randomize:
